...
def view_attendance():
    ...
    if school:
        profiles_query = profiles_query.filter(Profile.school.in_(school))
    if grade:
        profiles_query = profiles_query.filter(Profile.grade.in_(grade))
    if gender:
        profiles_query = profiles_query.filter(Profile.gender.in_(gender))
    if team:
        profiles_query = profiles_query.filter(Profile.team.in_(team))

    profiles = profiles_query.order_by(Profile.grade.asc(), Profile.name.asc()).all()
    records = []
    for profile in profiles:
        attendance = Attendance.query.filter_by(date=selected_date, event=selected_event, profile_id=profile.id).first()
        records.append((profile, attendance))
...
def register_attendance_event():
    ...
    profiles = query.order_by(Profile.grade.asc(), Profile.name.asc()).all()
    ...
    if request.method == "POST":
        ...
        exists = db.session.query(Attendance).filter_by(date=date, event=event).first()
        if exists:
            return render_template("attendance_register.html",
